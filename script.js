...
console.log(arr.slice()); //  = console.log([...arr]);
console.log(arr);
...
console.log(...arr.slice(-1));
console.log(arr.at(this.length - 1));
...
// for (const movement of movements) {
  for (const [i, movement] of movements.entries()) {
    if (movement > 0) {
      ...
      console.log(`Movement ${i + 1}: You whitdrew ${Math.abs(movement)}`);
    }
  }
  ...
  movements.forEach(function (mov, index, arr) {
    if (mov > 0) {
      ...
      console.log(`Movement ${index + 1}: You whitdrew ${Math.abs(mov)}`);
    }
  });
  // 0: function(200)
  // 1: function(450)
...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  const dogsTotal = dogsJuliaCorrect.concat(dogsKate);
  // console.log(dogsTotal);

  dogsTotal.forEach(function (age, num) {
    if (age >= 3) {
...
const calcAverageHumanAge = function (dogsJulia, dogsKate) {
  ...
  dogsJuliaCorrect.splice(-2);
  const dogsTotal = dogsJuliaCorrect.concat(dogsKate);
  const dogsAgeHumanYears = dogsTotal.map(age =>
    age <= 2 ? age * 2 : 16 + age * 4
  );
  console.log(dogsAgeHumanYears);
  const adultDogs = dogsAgeHumanYears.filter(age => age >= 18);
  console.log(adultDogs);
  const avgAgeDogsInHuAge =
    adultDogs.reduce((acc, dogAge) => acc + dogAge, 0) / adultDogs.length;
  console.log(avgAgeDogsInHuAge);
};
...
calcAverageHumanAge(juliaData1, kateData1);
console.log('\n');
...
// Chaining methods
const totalDepostisUSD = movements
  .filter(mov => mov > 0)
  // .map((mov, i, arr) => {
  // console.log(arr);
  // return mov * eurToUsd;
  ...
  console.log(totalDepostisUSD);
  console.log(movements);
  
  // Checks for equality
  console.log(movements.includes(-130));
  
  // Checks for conditioning
  // SOME
